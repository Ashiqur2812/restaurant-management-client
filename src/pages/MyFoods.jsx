...
const MyFoods = () => {
    ...
    const fetchAllFoods = async () => {
        const { data } = await axiosSecure.get(`/all-foods/${user?.email}`, { withCredentials: true });
        setFoods(data);
    };
                            ...
                            {foods.map(food => (
                                <tr key={food._id} className='text-center hover:bg-gray-100 transition-all animate__animated animate__fadeInUp'>
                                    ...
                                    <td className='border'>
                                        ...
                                        ${food.foodCategory === 'Beverage' && 'text-yellow-600 bg-yellow-100 rounded-full'}`}>
                                            {food.foodCategory}
                                        </p>
                                    </td>
                                    <td className='border py-3'>
                                        <div className='flex gap-3 items-center justify-center'>
                                            <Link to={`/update/${food._id}`} className="hover:text-blue-500 transition-colors">
                                                <div>
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 text-gray-600 hover:text-blue-500 hover:scale-125 transition-transform duration-300">
